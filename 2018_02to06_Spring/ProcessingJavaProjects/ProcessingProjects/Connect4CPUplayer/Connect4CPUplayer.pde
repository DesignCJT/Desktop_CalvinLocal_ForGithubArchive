...
class SqFillStuff {
  ...
  void clickyUse (float cxSpot, float cySpot) {
...
//ADDING Optimizer
            ...
            for (int c =0; c < moveOptions.length; c++){
              ...
              if (localNearby(testSpace)){}//close if
                ...
                perNotSmartMove = int(random(0,10));
              ...
              //if it's the default -1 that's regarded as unplayable and is set to reselect until not a -1 in the array
                ...
                moveSelection = int(random(0,totalMoves));
 ...
 boolean localNearby(int spaceToTest){
     int switchFlip = 0; 
...
//WIN DIA L WORK _______________________
  ...
  boolean checkIfBotMo(int testSq){
  ...
  void assignRowCol(int blockNum){
  ...
  void findWhichOne (int whichOne){
  ...
  void aiChangeFillType() {
    this.fillType = 2;
    this.activated = 1;
  }
  
  void ifInChangeFillType(int xSpot, int ySpot) {
  ...
  void displayGridPc (float boxSizeUse) {
